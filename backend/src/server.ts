...
app.post("/api/articles",async(req,res)=>{
  ...
  const existing=parsed.data.id?await findArticle(parsed.data.id):undefined;const selectedBias=parsed.data.mediaBiasId?await findBias(parsed.data.mediaBiasId):undefined;const now=new Date().toISOString();const settings={mediaBias:selectedBias?.name||parsed.data.mediaBias,mediaBiasId:selectedBias?.id||parsed.data.mediaBiasId,mediaBiasPrompt:selectedBias?.prompt||existing?.settings.mediaBiasPrompt||"",biasIntensity:parsed.data.biasIntensity,criticalIntensity:parsed.data.criticalIntensity,excitement:parsed.data.excitement,outputLength:parsed.data.outputLength,audience:parsed.data.audience,platform:parsed.data.platform};
  ...
  const article:NewsArticle={id:existing?.id||nanoid(12),subject:parsed.data.subject,headline:parsed.data.headline,lead:parsed.data.lead,body:parsed.data.body,settings,sources,versions:[version,...(existing?.versions||[])],threadId:existing?.threadId,status:parsed.data.status||"draft",createdAt:existing?.createdAt||now,updatedAt:now};
...
function publicArticle(article:NewsArticle){return {id:article.id,subject:article.subject,headline:article.headline,lead:article.lead,body:article.body,mediaBias:article.settings.mediaBias,biasIntensity:article.settings.biasIntensity,criticalIntensity:article.settings.criticalIntensity,excitement:article.settings.excitement,outputLength:article.settings.outputLength,audience:article.settings.audience,platform:article.settings.platform,status:article.status,updatedAt:new Date(article.updatedAt).getTime(),createdAt:article.createdAt,versions:article.versions.length};}
